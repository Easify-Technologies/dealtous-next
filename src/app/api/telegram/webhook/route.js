...
export async function POST(req) {
  try {
    ...
    if (message.chat_shared) {
      ...
      // 👉 Find latest product of this user (or use better logic)
      const product = await prisma.product.findFirst({
        where: {
          vendorId: user.id,
          telegramChannelId: null, // not yet linked
        },
        orderBy: {
          createdAt: "desc",
        },
      ...
      if (!product) {
        await sendMessage(
          message.chat.id,
          "❌ No pending product found to attach this channel.",
        );
        return NextResponse.json({ ok: true });
      }
      ...
      await sendMessage(
        message.chat.id,
        "✅ Channel successfully linked to your product!",
      );
    }
